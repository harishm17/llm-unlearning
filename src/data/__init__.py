...
from data.unlearn import ForgetRetainDataset, MTForgetRetainDataset
from data.pretraining import PretrainingDataset, CompletionDataset
from data.mt_jsonl_dataset import MTForgetJSONLDataset
...
def get_data(data_cfg: DictConfig, mode="train", **kwargs):
    ...
    if mode == "train":
        ...
        if "mt_forget" in unlearn_splits:
            # Multi-turn path: separate mt_forget out and pass explicitly so
            # MTForgetRetainDataset doesn't forward it as an unknown kwarg to
            # ForgetRetainDataset.__init__.
            mt_forget = unlearn_splits.pop("mt_forget")
            tokenizer = kwargs.get("tokenizer")
            unlearn_dataset = MTForgetRetainDataset(
                **unlearn_splits, mt_forget=mt_forget, tokenizer=tokenizer, anchor=anchor
            )
        else:
...
_register_data(MTForgetRetainDataset)
_register_data(MTForgetJSONLDataset)
